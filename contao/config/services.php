...
$container['doctrine.cache.impl.default'] = function () {
    if (array_key_exists('dbCache_' . TL_MODE, $GLOBALS['TL_CONFIG'])) {
        ...
        if (array_key_exists('dbCache', $GLOBALS['TL_CONFIG'])) {
...
$container['doctrine.cache.ttl.default'] = function () {
    if (array_key_exists('dbCacheTTL_' . TL_MODE, $GLOBALS['TL_CONFIG'])) {
        ...
        if (array_key_exists('dbCacheTTL', $GLOBALS['TL_CONFIG'])) {
...
$container['doctrine.cache.key.default'] = function () {
    if (array_key_exists('dbCacheName_' . TL_MODE, $GLOBALS['TL_CONFIG'])) {
        ...
        if (array_key_exists('dbCacheName', $GLOBALS['TL_CONFIG'])) {
    ...
    function ($container) {
        ...
        if (array_key_exists('TL_HOOKS', $GLOBALS) &&
            ...
            foreach ($GLOBALS['TL_HOOKS']['prepareDoctrineEventManager'] as $callback) {
                $object = method_exists($callback[0], 'getInstance') ? call_user_func(array(
                    $callback[0],
                    'getInstance'
                )) : new $callback[0];
...
$container['doctrine.connection.default'] = $container->share(
    function ($container) {
        ...
        switch (strtolower($GLOBALS['TL_CONFIG']['dbDriver'])) {
            ...
            default:
                throw new RuntimeException(
                    'Database driver ' . $GLOBALS['TL_CONFIG']['dbDriver'] . ' not known by doctrine.'
                );
        }
        ...
        if (array_key_exists('TL_HOOKS', $GLOBALS)
            && array_key_exists('prepareDoctrineConnection', $GLOBALS['TL_HOOKS'])
            && is_array($GLOBALS['TL_HOOKS']['prepareDoctrineConnection'])
        ) {
        ...
        if (array_key_exists('TL_HOOKS', $GLOBALS)
            && array_key_exists('doctrineConnect', $GLOBALS['TL_HOOKS'])
            && is_array($GLOBALS['TL_HOOKS']['doctrineConnect'])
        ) {
